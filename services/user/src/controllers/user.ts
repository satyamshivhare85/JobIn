...
export const updatedProfilePic = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    console.log("UPDATED PROFILE PIC HIT");
    ...
    if (!file) {
      throw new ErrorHandler(400, "Profile pic required");
    }
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    if (!wasSkillAdded) {
      return res.status(400).json({
        message: `Skill ${skillName.trim()} already exists for this user`,
      });
    }
...
res.json({
  message:"skill is succesfully deleted",
